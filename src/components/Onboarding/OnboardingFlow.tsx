  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
      <div className="w-full max-w-lg bg-white rounded-[2rem] shadow-2xl shadow-indigo-100 border border-white p-6 md:p-10 relative overflow-hidden">
        ...
        {step < 4 && (
          <div className="mb-8 flex items-center justify-between">
            <button 
              disabled={step === 1}
              onClick={prevStep}
              className={`p-2 rounded-full hover:bg-slate-100 transition-colors ${step === 1 ? 'opacity-0 cursor-default' : 'text-slate-600'}`}
            >
              <ArrowLeft size={20} />
            </button>
            <span className="font-bold text-indigo-600 text-sm tracking-wider uppercase">
        ...
        <AnimatePresence mode='wait'>
          {step === 1 && (
            <Step1Profile key="step1" data={formData} updateData={updateData} onNext={nextStep} />
          )}
          {step === 2 && (
            <Step2Habits key="step2" data={formData} updateData={updateData} onNext={nextStep} onBack={prevStep} />
          )}
          {step === 3 && (
            <Step3Status key="step3" data={formData} updateData={updateData} onNext={nextStep} onBack={prevStep} />
          )}
          {step === 4 && (
            <Step4Success key="step4" onComplete={() => onFinish(formData)} />
          )}
